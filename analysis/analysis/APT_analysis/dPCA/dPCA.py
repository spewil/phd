...
class dPCA(BaseEstimator):
    ...
    def _marginalize(self, X, save_memory=False):
        ...
        # center data
        Xres -= np.mean(Xres.reshape((Xres.shape[0], -1)),
                        -1).reshape((Xres.shape[0], ) + (len(Xres.shape) - 1) *
                                    (1, ))
        ...
        if save_memory:
            for key, phi in pcombs.items():
                ...
                Xmargs[key] = mmean(Xres,
                                    np.array(S_without_phi) + 1,
                                    expand=True)
    ...
    def _optimize_regularization(self, X, trialX, center=True, lams='auto'):
        ...
        if np.argmin(totalscore) == 0 or np.argmin(
                totalscore) == len(totalscore) - 1:
            if self.debug > 0:
    ...
    def crossval_score(self, lams, X, trialX, mean=True):
        ...
        for trial in range(self.n_trials):
            ...
            trainX, validX = self.train_test_split(X,
                                                   trialX,
                                                   N_samples=N_samples)
    ...
    def _score(self, X, mXs, mean=True):
        ...
        trPD = {
            key: np.sum(self.P[key] * self.D[key], 1)
            for key in list(mXs.keys())
        }
    ...
    def _fit(self,
        ...
        # center data
        if center:
            X = X - np.mean(flat2d(X), 1).reshape((n_features, ) +
                                                  len(self.labels) * (1, ))
        ...
        if self.regularizer > 0:
            regX, regmXs, pregX = self._add_regularization(X,
                                                           mXs,
                                                           self.regularizer *
                                                           np.sum(X**2),
                                                           SVD=SVD)
    ...
    def _zero_mean(self, X):
        ...
        return X - np.mean(X.reshape(
            (X.shape[0], -1)), 1).reshape((X.shape[0], ) + (len(X.shape) - 1) *
                                          (1, ))
    ...
    def _get_n_samples(self, trialX, protect=None):
        ...
        return trialX.shape[0] - np.sum(
            np.isnan(trialX[(np.s_[:], ) + (np.s_[:], ) * n_unprotect +
                            (0, ) * n_protect]), 0)
    ...
    def _check_protected(self, X, protect):
        ...
        if protect is None:
            ...
            if set(protect) == set(
                    np.arange(
                        len(self.labels) - len(protect), len(self.labels))):
                protected = True
            else:
                protected = False
                print(
    ...
    def train_test_split(self, X, trialX, N_samples=None, sample_ax=0):
        ...
        n_unprotect = len(
            X.shape) - len(protect) if protect is not None else len(X.shape)
        n_protect = len(protect) if protect is not None else 0
        ...
        while not it.finished:
            blindX[it.multi_index +
                   (np.s_[:], ) * n_protect] = trialX[(idx[it.multi_index], ) +
                                                      it.multi_index +
                                                      (np.s_[:], ) * n_protect]
            it.iternext()
        ...
        # compute trainX
        trainX = (X * (N_samples /
                       (N_samples - 1))[(np.s_[:], ) * n_unprotect +
                                        (None, ) * n_protect] - blindX /
                  (N_samples - 1)[(np.s_[:], ) * n_unprotect +
                                  (None, ) * n_protect])
        ...
        trainX -= np.mean(flat2d(trainX),
                          1)[(np.s_[:], ) + (None, ) * (len(X.shape) - 1)]
        blindX -= np.mean(flat2d(blindX),
                          1)[(np.s_[:], ) + (None, ) * (len(X.shape) - 1)]
    ...
    def significance_analysis(self,
                              X,
                              trialX,
                              n_shuffles=100,
                              n_splits=100,
        ...
        def compute_mean_score(X, trialX, n_splits):
            ...
            if type(self.n_components) == int:
                scores = {
                    key: np.empty((self.n_components, K))
                    for key in keys
                }
            else:
                scores = {
                    key: np.empty((self.n_components[key], K))
                    for key in keys
                }
            ...
            for shuffle in range(n_splits):
                ...
                for key in keys:
                    ...
                    for comp in range(ncomps):
                        scores[key][comp] = nan_shuffle.classification(
        ...
        for it in range(n_shuffles):
            print("\rCompute score of shuffled data: ",
                  str(it),
                  "/",
                  str(n_shuffles),
                  end=' ')
        ...
        if n_consecutive > 1:
            for key in keys:
                ...
                for k in range(mask.shape[0]):
                    masks[key][k, :] = nan_shuffle.denoise_mask(
    ...
    def transform(self, X, marginalization=None):
        ...
        if marginalization is not None:
            ...
            X_transformed = np.dot(D.T,
                                   Xr).reshape((D.shape[1], ) + X.shape[1:])
            ...
            for key in list(self.marginalizations.keys()):
                X_transformed[key] = np.dot(
                    self.D[key].T, X.reshape(
                        (X.shape[0],
                         -1))).reshape((self.D[key].shape[1], ) + X.shape[1:])
    ...
    def inverse_transform(self, X, marginalization):
        ...
        X_transformed = np.dot(
            self.P[marginalization], X.reshape(
                (X.shape[0],
                 -1))).reshape((self.P[marginalization].shape[0], ) +
                               X.shape[1:])
